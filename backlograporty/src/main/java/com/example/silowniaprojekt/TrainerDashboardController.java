...
public class TrainerDashboardController {
    ...
    @FXML private Button trainingPlansBtn;
    ...
    private static class ProductFormData {
        ...
        public ProductFormData(String name, BigDecimal price, int stock) {
            this.name = name;
            this.price = price;
            this.stock = stock;
        }
    ...
    public void initialize() {
        ...
        // Dodajemy podświetlanie do listenerów tabel
        if (pendingRequestsTable != null) {
            pendingRequestsTable.getSelectionModel().selectedItemProperty().addListener((obs, oldSel, newSel) -> setActiveNavButton(trainingPlansBtn));
        }
        if (trainingScheduleTable != null) {
            trainingScheduleTable.getSelectionModel().selectedItemProperty().addListener((obs, oldSel, newSel) -> setActiveNavButton(scheduleBtn));
        }
        if (incomeTable != null) {
            incomeTable.getSelectionModel().selectedItemProperty().addListener((obs, oldSel, newSel) -> setActiveNavButton(incomeBtn));
        }
        if (tasksProgressTable != null) {
            tasksProgressTable.getSelectionModel().selectedItemProperty().addListener((obs, oldSel, newSel) -> setActiveNavButton(progressBtn));
        }
        if (reportsTable != null) {
            reportsTable.getSelectionModel().selectedItemProperty().addListener((obs, oldSel, newSel) -> setActiveNavButton(reportsBtn));
        }
        if (productsTable != null) {
            productsTable.getSelectionModel().selectedItemProperty().addListener((obs, oldSel, newSel) -> setActiveNavButton(productsBtn));
        }
    ...
    private void initializeIncomeTable() {
        ...
        incomeTrainingDateTimeColumn.setCellFactory(tc -> new TableCell<TrainerIncomeEntry, LocalDateTime>() {
            @Override
            protected void updateItem(LocalDateTime item, boolean empty) {
                ...
                if (empty || item == null) {
                    ...
                    setText(item.toLocalDate().toString()); // lub format daty jeśli potrzebny
        ...
        incomePaymentDateColumn.setCellFactory(tc -> new TableCell<TrainerIncomeEntry, Timestamp>() {
            @Override
            protected void updateItem(Timestamp item, boolean empty) {
                ...
                if (empty || item == null) {
                    ...
                    setText(item.toString()); // lub format daty jeśli potrzebny
                    setStyle("-fx-alignment: CENTER-RIGHT; -fx-padding: 0 15px 0 0;");
                }
            }
        });
        incomeAmountColumn.setCellValueFactory(new PropertyValueFactory<>("amount"));
        incomeAmountColumn.setCellFactory(tc -> new TableCell<TrainerIncomeEntry, BigDecimal>() {
            @Override
    ...
    private void loadIncomeReports() {
        ...
        totalIncomeLabel.setText(String.format("%.2f zł", totalIncome));
    }
    ...
    private void initializeProgressSection() {
        ...
        taskTrainingDateColumn.setCellFactory(tc -> new TableCell<TrainerTask, LocalDate>() {
            @Override
        ...
        statusesList = FXCollections.observableArrayList();   // Inicjalizacja (dane za��adujesz w loadPrioritiesAndStatuses)
    ...
    private void initializeTrainingPlans() { // Nazwa odzwierciedla "Ustal trening"
        ...
        planDatePicker.setDayCellFactory(picker -> new DateCell() {
            @Override
        ...
        planDatePicker.setDayCellFactory(picker -> new DateCell() {
            ...
            public void updateItem(LocalDate date, boolean empty) {
                ...
                // Wyłącz dni w przeszłości
                if (date.compareTo(LocalDate.now()) < 0) {
                    setDisable(true);
                    setStyle("-fx-background-color: #ffc0cb;"); // Różowy dla przeszłości
                }

                // Sprawdź czy data jest zajęta w harmonogramie
                boolean isDateOccupied = trainingScheduleList.stream()
                        .anyMatch(entry -> entry.getTrainingDate().equals(date));

                if (isDateOccupied) {
                    setDisable(true);
                    setStyle("-fx-background-color: #ff9999;"); // Czerwony dla zajętych
                    setTooltip(new Tooltip("Masz już trening w tym dniu!"));
                }
            }
        });
    ...
    private void initializeSchedule() {
        ...
        scheduleDateColumn.setCellFactory(column -> new TableCell<TrainingScheduleEntry, LocalDate>() { // Zmieniono na TrainingScheduleEntry i LocalDate
            private final DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd"); // Zmieniono format na odpowiedni dla LocalDate

            @Override
            protected void updateItem(LocalDate item, boolean empty) { // Zmieniono typ parametru na LocalDate
                super.updateItem(item, empty);
                if (empty || item == null) {
                    setText(null);
                } else {
    ...
    private void initializeProductsTable() {
        ...
        productPriceColumn.setCellFactory(tc -> new TableCell<Product, BigDecimal>() {
            @Override
    ...
    public void showTrainingPlans() { // Nazwa metody odzwierciedla "Ustal trening"
        ...
        setActiveNavButton(trainingPlansBtn);
    ...
    public void showProducts() {
        ...
        setActiveNavButton(productsBtn);
    ...
    public void handleSaveTrainingPlan() {
        ...
        if (trainingDate.isBefore(LocalDate.now())) {
            showError("Data treningu nie może być w przeszłości.");
            return;
        }
        ...
        boolean isDateOccupied = trainingScheduleList.stream()
                .anyMatch(entry -> entry.getTrainingDate().equals(trainingDate));

        if (isDateOccupied) {
            showError("Masz już zaplanowany trening w tym dniu! Wybierz inną datę.");
            return;
        }
                ...
                if (affectedRowsInsert > 0) {
                    ...
                    if (generatedTrainingRequestId != -1) {
                        ...
                        int defaultPriorityId = prioritiesList.stream()
                                .filter(p -> p.getName().equals("Średni"))
                                .map(Priority::getId)
                                .findFirst()
                                .orElseThrow(() -> new SQLException("Default Priority 'Średni' not found in loaded data. Ensure loadPrioritiesAndStatuses() is called."));
    ...
    private void handleAddProduct() {
        ...
        grid.addRow(1, new Label("Cena:"), priceField);
        grid.addRow(2, new Label("Ilość na stanie:"), stockField);
        ...
        AtomicReference<ProductFormData> formDataRef = new AtomicReference<>();

        dialog.setResultConverter(button -> {
            if (button == addButtonType) {
                try {
                    formDataRef.set(new ProductFormData(
                            nameField.getText(),
        ...
        if (result.isPresent() && result.get() == addButtonType) {
            ProductFormData formData = formDataRef.get();
            if (formData != null) {
                String sql = "INSERT INTO products(name, price, stock) VALUES (?, ?, ?)";
                try (Connection conn = DatabaseConnection.getConnection();
                     PreparedStatement pstmt = conn.prepareStatement(sql)) {

                    pstmt.setString(1, formData.name);
                    pstmt.setBigDecimal(2, formData.price);
                    pstmt.setInt(3, formData.stock);
                    pstmt.executeUpdate();
                    loadProducts(); // Odśwież tabelę

                } catch (SQLException | NumberFormatException e) {
                    showError("Błąd dodawania produktu: " + e.getMessage());
                }
            }
        }
    }
    ...
    private void handleEditProduct() {
        ...
        dialog.setHeaderText("Zmień dane produktu");
        ...
        ButtonType editButtonType = new ButtonType("Zapisz", ButtonBar.ButtonData.OK_DONE);
        dialog.getDialogPane().getButtonTypes().addAll(editButtonType, ButtonType.CANCEL);

        // Używamy AtomicReference, aby zapisać dane z lambdy
        AtomicReference<ProductFormData> formDataRef = new AtomicReference<>();
        ...
        dialog.setResultConverter(button -> {
            if (button == editButtonType) {
                ...
                return button; // Zawsze zwracamy ButtonType
            }
            return null;
        });
        ...
        if (result.isPresent() && result.get() == editButtonType) {
            ...
            if (formData != null) {
                ...
                try (Connection conn = DatabaseConnection.getConnection();
                    ...
                    showError("Błąd edycji produktu: " + e.getMessage());
                }
            }
        }
    }
    ...
    private void setActiveNavButton(Button activeBtn) {
        ...
        trainingPlansBtn.getStyleClass().remove("active");
        scheduleBtn.getStyleClass().remove("active");
        incomeBtn.getStyleClass().remove("active");
        progressBtn.getStyleClass().remove("active");
    ...
    public void logout() {
        try {
            ...
            // Przekazanie referencji do loginStage do LoginController
