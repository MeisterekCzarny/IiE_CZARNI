...
public class ClientDashboardController {
    ...
    private void initializeProfile() {
        ...
        taskDateColumn.setCellFactory(column -> new TableCell<ClientTask, java.time.LocalDate>() {
    ...
    private void initializeClientActivities() {
        ...
        activityAmountColumn.setCellFactory(column -> new TableCell<ClientActivity, BigDecimal>() {
            @Override
            protected void updateItem(BigDecimal item, boolean empty) {
                ...
                if (empty || item == null) {
                    ...
                    // Use String.format similar to the AdminDashboardController example
                    // Locale.US is used to ensure '.' as the decimal separator.
        ...
        activityDateColumn.setCellFactory(column -> new TableCell<ClientActivity, Timestamp>() {
    ...
    private void initializeClientTrainingsRequest() {
        ...
        trainingDateColumn.setCellFactory(column -> new TableCell<ClientTrainingRequest, java.sql.Date>() {
            private final DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
            @Override
    ...
    public void showMembership() {
        ...
        setActiveNavButton(membershipBtn);
    }
    ...
    public void showTrainings() { // To jest sekcja "Zapisz się na Trening"
        ...
        loadClientTrainingRequests();
        setActiveNavButton(trainingsBtn);
    }
    ...
    public void showClientActivities() {
        ...
        setActiveNavButton(activitiesBtn);
    }
    ...
    private void setActiveNavButton(Button activeBtn) {
        ...
        membershipBtn.getStyleClass().remove("active");
        trainingsBtn.getStyleClass().remove("active");
        shopBtn.getStyleClass().remove("active");
        scheduleBtn.getStyleClass().remove("active");
        activitiesBtn.getStyleClass().remove("active");
        changePasswordBtn.getStyleClass().remove("active");
    ...
    private void logout() {
        try {
            ...
            loginStage.setTitle("Black Iron Gym - System logowania");
    ...
    public void changePassword(ActionEvent event) {
        ...
        if (userId == 0) {
            showError( "Nie można zmienić hasła - użytkownik nie został poprawnie załadowany.");
            return;
        }

        // Generowanie hasha BCrypt
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            if (affectedRows > 0) {
                ...
                showError( " Nie udało się zmienić hasła. Spróbuj ponownie.");
            }

        } catch (SQLException e) {
            showError( "Nie można było zmienić hasła.");
            e.printStackTrace();
        }
    }

    // Dodaj to jako metodę prywatną w klasie
